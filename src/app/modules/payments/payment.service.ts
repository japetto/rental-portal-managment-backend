  ...
  try {
    ...
    if (overduePayments.length > 0) {
      ...
      paymentOptions.push({
        ...
        dueDate: overduePayments[0]?.dueDate,
      });
    }
    ...
    if (isFirstTimePayment) {
      ...
      const hasFirstMonthPayment = paymentHistory.some(
        payment => payment.dueDate.getTime() === leaseStart.getTime(),
      );
      ...
      if (hasFirstMonthPayment && hasSecondMonthPayment) {
        ...
        const secondMonthName = new Date(
          leaseStart.getFullYear(),
          leaseStart.getMonth() + 1,
          1,
      ...
      currentMonthPayment?.status === PaymentStatus.CANCELLED ||
      currentMonthPayment?.status === PaymentStatus.REFUNDED
    ) {
    ...
    const isLeaseExpiringSoon = activeLease.leaseEnd
      ? Math.floor(
          (activeLease.leaseEnd.getTime() - currentDate.getTime()) /
            (1000 * 60 * 60 * 24),
        ) <= 30
      : false;
    ...
    return {
      ...
      leaseExpirationWarning:
        isLeaseExpiringSoon && activeLease.leaseEnd
          ? `Your lease expires on ${activeLease.leaseEnd.toLocaleDateString(
              "en-US",
              {
                year: "numeric",
                month: "long",
...
const verifyPaymentLinkOwnership = async (
...
const getPendingPaymentDetails = async (tenantId: string) => {
  try {
    ...
    const tenant = await Users.findById(tenantId);
    const property = await Properties.findById(pendingPayment.propertyId);
    const spot = await Spots.findById(pendingPayment.spotId);
...
const createPaymentWithLink = async (paymentData: {
  ...
  try {
    ...
    const pendingPaymentDetails = await getPendingPaymentDetails(
...
const getReceiptBySessionId = async (sessionId: string, accountId?: string) => {
  try {
    ...
    let payment = await Payments.findById(
