            ...
            if (lease) {
                ...
                if (isFirstTimePayment) {
                    if (this.amount > totalRentAmount + lease.depositAmount) {
                        return next(new Error(`First-time payment amount (${this.amount}) cannot exceed total rent + deposit (${totalRentAmount + lease.depositAmount})`));
                    }
                }
                else {
                    // For regular rent payments, amount should not exceed monthly total rent
                    if (this.amount > totalRentAmount) {
                        return next(new Error(`Rent payment amount (${this.amount}) cannot exceed lease total rent amount (${totalRentAmount})`));
                    }
                }
            }
        }
