...
const handleStripeWebhook = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        // If no accountId found, we'll need to identify it from the webhook endpoint
        if (!accountId) {
            console.log("🔧 Legacy: No accountId found in request, will identify from webhook endpoint");
        }
        // Get the account with the webhook secret
        let stripeAccount = null;
        if (accountId) {
            ...
            console.log("🔧 Legacy: Account lookup result:", {
                accountId,
                found: !!stripeAccount,
                hasWebhookSecret: !!(stripeAccount === null || stripeAccount === void 0 ? void 0 : stripeAccount.webhookSecret),
                accountName: stripeAccount === null || stripeAccount === void 0 ? void 0 : stripeAccount.name,
            });
        }
        else {
        ...
        console.log("🔍 Legacy webhook - Request body is Buffer:", Buffer.isBuffer(req.body));
        if (Buffer.isBuffer(req.body)) {
            ...
            rawBody = Buffer.from(req.body, "utf8");
            console.log("🔧 Legacy: Using string converted to buffer for webhook verification");
        }
        else if (typeof req.body === "object" && req.body !== null) {
            ...
            const jsonString = JSON.stringify(req.body, null, 0); // No pretty formatting
            rawBody = Buffer.from(jsonString, "utf8");
            console.log("🔧 Legacy: Using reconstructed buffer from parsed JSON for webhook verification");
        }
        else {
        ...
        if (!stripeAccount.webhookSecret) {
            ...
            console.log(`🔧 Legacy: Found ${allAccounts.length} accounts with webhook secrets`);
            // We'll try each account's webhook secret
            for (const account of allAccounts) {
                try {
                    const testStripe = new stripe_1.default(account.stripeSecretKey, {
                    ...
                    const testEvent = testStripe.webhooks.constructEvent(rawBody, signature, account.webhookSecret);
                    // If we get here, the verification succeeded
                    console.log(`✅ Legacy: Found matching account: ${account._id} (${account.name})`);
                    stripeAccount = account;
                    accountId = account._id.toString();
                    break;
                }
                catch (verificationError) {
                    console.log(`❌ Legacy: Account ${account._id} verification failed: ${verificationError.message}`);
                    continue;
                }
            }
        }
        ...
        console.log(`🔔 LEGACY WEBHOOK RECEIVED: ${event.type} for account ${accountId}`);
        // Handle the event based on type
        switch (event.type) {
            case "payment_intent.succeeded":
                yield handleSuccessfulPayment(event.data.object, stripeAccount);
        ...
        return res.status(400).json({ error: `Legacy Webhook Error: ${error.message}` });
    }
});
